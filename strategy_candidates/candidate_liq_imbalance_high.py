...
def strategy_liq_imbalance_high_regime(market, secs_left, tracker, position):
    """
    AUTO-GENERATED CANDIDATE — 2026-03-26 20:22:24
    ---------------------------------------------------------
    Source: ML regime analysis
    Feature:     liq_imbalance
    Regime:      HIGH (above median threshold)
    Threshold:   0.000000  (approximate median from training data)
    WF edge:     +0.1523 Brier improvement vs baseline
    Sample size: 332 test rows

    STATUS: OBSERVATION MODE — not validated for live trading
    ---------------------------------------------------------
    What this means:
    When liq_imbalance is HIGH (> 0.0000),
    the ML model predicts crowd direction more accurately (+0.1523 Brier).
    ...
    - Feature liq_imbalance maps to: _regime / _vol_cache / _funding_cache
    - Consider combining with other regime filters for stronger signal
    """
    # TODO: Get the signal value for liq_imbalance
    ...
    if signal_value <= 0.0000:
        _log_signal("ML_liq_imbalance_HIGH", market, secs_left,
                    signal_value=signal_value,
                    threshold=0.0000,
                    reason="liq_imbalance_regime_filter")
        return position

    # Strategy fires in HIGH liq_imbalance regime
